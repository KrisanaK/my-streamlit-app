...
def validate_test_plan(df, filename, settings):
    ...
    if settings["check_branch_fail"]:
        ...
        if not non_fail_rows.empty:
            errors.append(f"{filename}: {len(non_fail_rows)} test(s) Sort ≠ 'FAIL'.")

    if settings["check_hfe_ar"]:
        for _, row in df.iterrows():
            if str(row['ITEM']).strip().upper() == "HFE" and row['AR'] != "AR":
                errors.append(f"{filename}: HFE test NO {row['NO']} missing AR option.")
...
def check_sort_coverage(df, bin_rows, filename):
    ...
    if missing:
        errors.append(f"{filename}: Missing sort plan coverage for: {', '.join(sorted(missing))}")
    return errors
...
def load_mapping(file_path='paper_specs/mtm_product_map.csv'):
    return pd.read_csv(file_path) if os.path.exists(file_path) else pd.DataFrame(columns=['Filename', 'Product'])
...
def save_spec(df_spec, product_filename):
    save_dir = 'paper_specs'
    os.makedirs(save_dir, exist_ok=True)
    df_spec.to_csv(os.path.join(save_dir, product_filename), index=False)
...
with tabs[0]:
    ...
    settings = {
        ...
        "check_single_pass": st.sidebar.checkbox("Only one PASS bin"),
        "check_coverage": st.sidebar.checkbox("Check test coverage"),
        "check_bin_out": st.sidebar.checkbox("BIN OUT required"),
        "check_osc": st.sidebar.checkbox("OSC required"),
        "check_spec_limits": st.sidebar.checkbox("Validate test vs spec"),
    }

    required_bin = st.sidebar.text_input("PASS bin =", "11").zfill(2) if settings["check_pass_format"] else None
...
with tabs[1]:
    ...
    st.subheader("MTM ↔ Product Spec Mapping")
    ...
    product_selected = st.selectbox("Select Product to Edit", options=products)

    if product_selected:
        spec_df = load_spec(product_selected)
        st.subheader(f"Editing Spec: {product_selected}")
        ...
        if st.button("Save Paper Spec"):
            ...
            st.success(f"Spec '{product_selected}' saved!")
